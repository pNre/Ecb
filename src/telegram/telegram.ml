...
let send_message
  ...
  Deferred.Result.all (split_and_send text)
;;
...
let get_file id =
  ...
  >>| map_body
  >>| Result.map_error ~f:(fun _ -> Http.Response (uri, response, body))
;;
...
(* Utils *)
type update_type =
  | Command of string * string list * int64
  | Unknown
[@@deriving yojson { strict = false }]
...
let parse_update update =
  ...
  | { message = Some { chat = { id = chat_id; _ }; text = Some t; _ }; _ } ->
    ...
    | command :: args ->
      let command = command |> String.strip |> String.lowercase in
      let args = args |> List.map ~f:String.strip in
      Command (command, args, chat_id)
    | _ -> Unknown)
  | _ -> Unknown
;;

let handle_failure chat_id err =
  send_message ~chat_id ~text:err ()
  >>|? ignore
  >>| Result.map_error ~f:Http.string_of_error
;;

let handle_success chat_id text =
  send_message ~chat_id ~text () >>|? ignore >>| Result.map_error ~f:Http.string_of_error
;;
